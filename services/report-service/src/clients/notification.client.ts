...
export class NotificationClient {
  ...
  private get baseUrl() {
    return this.config.get<string>('NOTIFICATION_SERVICE_URL', 'http://localhost:3004');
  }

  async getNotifications(params: Record<string, string | undefined>) {
    const { data } = await firstValueFrom(
      this.http.get(`${this.baseUrl}/api/internal/notifications/report`, {
